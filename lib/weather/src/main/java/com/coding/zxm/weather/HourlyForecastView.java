...
public class HourlyForecastView extends View implements ScrollWatcher {
    ...
    private void initDefValue() {
        ...
        itemWidth = ScreenUtil.dp2px(mContext, 30);

        defWidthPixel = itemWidth * (ITEM_SIZE - 1);
        defHeightPixel = ScreenUtil.dp2px(mContext, 80);

        lowestTempHeight = ScreenUtil.dp2px(mContext, 40);//长度  非y轴值
        highestTempHeight = ScreenUtil.dp2px(mContext, 70);
        //defPadding
        paddingT = ScreenUtil.dp2px(mContext, 20);
        paddingL = ScreenUtil.dp2px(mContext, 10);
        paddingR = ScreenUtil.dp2px(mContext, 15);

        textSize = ScreenUtil.sp2px(mContext, 12);


        bitmapHeight = 1 / 2f * (2 * defHeightPixel - lowestTempHeight) + ScreenUtil.dp2px(mContext, 2);//- 给文字留地方
        bitmapXY = 18;
...
//        setLayerType(View.LAYER_TYPE_SOFTWARE, null);//关闭硬件加速
        ...
        textLinePaint.setTextSize(ScreenUtil.sp2px(getContext(), 12));
        textLinePaint.setAntiAlias(true);

        textLinePaint.setColor(mContext.getResources().getColor(R.color.colorBlack));
...
//                int Y = getTempBarY(i);
                ...
                Rect targetRect = new Rect(getScrollBarX(), Y - ScreenUtil.dp2px(getContext(), 24)
                        , getScrollBarX() + offset, Y - ScreenUtil.dp2px(getContext(), 4));
...
//            textPaint.setTextSize(ScreenUtil.sp2px(mContext, 8));
//            canvas.drawText(text, drawPoint, bitmapHeight + bitmap.getHeight() + 100 / 3f, textPaint);
        ...
        for (int i = 0; i < hourlyWeatherList.size(); i++) {
            ...
            if (ITEM_SIZE > 8) {
                if (i % 2 == 0) {
                    ...
                    canvas.drawText(time.substring(time.length() - 11, time.length() - 6), w, baseLineHeight + textSize + ScreenUtil.dp2px(mContext, 3), textPaint);
                }
            } else {
                textPaint.setTextAlign(Paint.Align.CENTER);
                if (i == 0) {
                    canvas.drawText(mContext.getString(R.string.now), w, baseLineHeight + textSize + ScreenUtil.dp2px(mContext, 3), textPaint);
                } else {
                    canvas.drawText(time.substring(time.length() - 11, time.length() - 6), w, baseLineHeight + textSize + ScreenUtil.dp2px(mContext, 3), textPaint);
                }
            }
    ...
    public void setScrollOffset(int offset, int maxScrollOffset) {
        this.maxScrollOffset = maxScrollOffset + ScreenUtil.dp2px(mContext, 50);
        scrollOffset = offset;
    ...
    private int getScrollBarX() {
        ...
        x = x - ScreenUtil.dp2px(mContext, 3);
        return x;
    }

    private static Bitmap bitmapResize(Bitmap src, float pxX, float pxY) {
        //压缩图片
